...
def _discord_post(content: str) -> bool:
    ...
    # Discord max content length is 2000; chunk if needed
    if len(content) <= 2000:
        data = {"content": content}
    else:
        # Split into chunks of 1990 to leave room for code block markers
        chunks = []
...
def _telegram_post(content: str, parse_mode: str = "MarkdownV2") -> bool:
    ...
    if len(content) <= MAX_MSG:
        data = {"chat_id": chat_id, "text": content, "parse_mode": parse_mode}
    else:
        ...
        for i in range(0, len(content), MAX_MSG):
            ...
            try:
                ...
                if resp.status_code != 200:
                    print(f"[telegram] chunk {i}: HTTP {resp.status_code}")
            except Exception as e:
                print(f"[telegram] chunk {i}: {e}")
        return True
