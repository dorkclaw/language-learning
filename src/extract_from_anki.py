...
INPUT = os.path.join(os.path.dirname(__file__), "../anki/Refold ES1K.apkg")  # Place your audio files here
OUTPUT_FOLDER = os.path.join(os.path.dirname(__file__), "../anki/", os.path.basename(INPUT)[:-5])  # Transcriptions will be saved here
PROMPT_FILE = os.path.join(os.path.dirname(__file__), "extract_from_anki_repackage_prompt.txt")  # File containing the instructions
...
def load_apkg_to_genanki(apkg_path):
    ...
    if os.path.exists(os.path.join(extract_dir, 'collection.anki21b')):
        raise ValueError("This .apkg file appears to be from a newer Anki version (2.1.21+). Export the deck in compatibility mode and try again")
...
def note_to_llm_str(note):
    ...
    fields_str = "\n".join([f"{field['name']}: {value}" for field, value in zip(model.fields, note.fields) if field['name'] not in deny_fields])
    assert len(note.tags) == 0, "This function does not currently handle tags. Please remove tags from the note or extend this function to include them."
...
def process_note(note, extraction_rules):
    output_filepath = os.path.join(OUTPUT_FOLDER, f"{b64_encode(note.guid)}.json")
    if os.path.exists(output_filepath):
        print(f"Note {note.guid} already processed, skipping.")
        return
    time.sleep(0.1 * MAX_CONCURRENT_CALLS * random.random())

    print(f"Processing note with GUID: {note.guid}")
    note_str = note_to_llm_str(note)
    ...
    try:
        ...
        ], print_reasoning=(MAX_CONCURRENT_CALLS <= 2))  # Only print reasoning if we're doing a single call for easier debugging
        
        # Save the formatted JSON to the output folder
        with open(output_filepath, "w", encoding="utf-8") as out_f:
            json.dump(vocab_data, out_f, indent=4, ensure_ascii=False)
            
        print(f"Successfully extracted {len(vocab_data)} words for {note.guid}.json.")

        #print(f"DeepSeek's reasoning process for {filename}:\n{reasoning_process}\n")

    except json.JSONDecodeError:
        print(f"JSON Error on {note.guid}: The model output invalid JSON format. Check the raw text.")
        ...
        print(f"API Error on {note.guid}: {e}")
   ...
   #         process_note(note, extraction_rules)
    ...
    for deck in decks:
        with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_CALLS) as executor:
            ...
            future_to_file = {
                executor.submit(process_note, note, extraction_rules): note 
